...
class WebSMSService {
  ...
  public function __construct() {
    $this->login = "dom_na_dereve";
    $this->password = "Barakuda1989";
  ...
  public function getStatus($message_id){
    ...
    $cost     = "";
    $d = getdate();
    $dd = str_pad($d['mday'], 2, '0', STR_PAD_LEFT).'.'.str_pad($d['mon'], 2, '0', STR_PAD_LEFT).'.'.$d['year'].' '.str_pad($d['hours'], 2, '0', STR_PAD_LEFT).':'.str_pad($d['minutes'], 2, '0', STR_PAD_LEFT).':'.str_pad($d['seconds'], 2, '0', STR_PAD_LEFT);
    try {
      ...
      if ($errCode==0) {
        ...
        $dd       = (string)$xml->httpOut->sms['delivered_date'] == '' ? $dd : (string)$xml->httpOut->sms['delivered_date'];
       }
    } catch (Exception $ex) {
      $errCode = -100;
