                    ...
                    <form method="POST" action="{{route('promocodes.store')}}"  enctype="multipart/form-data">
                        ...
                        <label class="form-label">Максимальное количество использований промокода:</label>
                            <div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="usable_count" value="999999999" checked>
                                    <label class="form-check-label">
                                        Без ограничений
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" id="usable_count" name="usable_count">
                                    <label class="form-check-label">
                                        Количество
                                        <input class="form-control" id="usable_count_changer" type="number" step="1" min="0" disabled>
                                    </label>
                        ...
                        <label class="form-label">Выберите тип скидки для промокода:</label>
                            <select class="form-select" id="promocode_type" name="promocode_type" required>
                                ...
                                <option value="{{$k}}">{{$promocode_type}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <label class="form-label">Задайте правило применения скидки:</label>
                            <div id="rule_type_changer">
                                ...
                                <input class="form-control" type="number" step="1" min="0" name="discount" required>
                                <span id="percent_discount_text" style="display: none">% на</span>
                                <span id="price_discount_text">руб. на</span>

                                <select class="form-select" name="rule_type" required>
                                    @foreach(config('enums.promocode_rules') as $k => $promocode_rule)
                                    <option value="{{$k}}">{{$promocode_rule}}</option>
                                    @endforeach
                                </select>

                                <div id="min_subtotal_wrapper" style="display:none">
                                    <input class="form-control" id="min_subtotal" type="number" step="1" min="0" name="min_subtotal">
                                    <span id="price_discount_text">руб.</span>
                                </div>

                                <div id="categories_wrapper" style="display:none">
                                ...
                                <div id="products_wrapper" style="display:none">
                                    <select id="products" name="products[]" multiple>
                                        @foreach($products as $product)
                                        <option value="{{$product->id}}">
                                            {{$product->product->getLocalizeTitleRu()}} 
                                            @if($product->color->id !== 76){{$product->color->getLocalizeTitleRu()}}@endif
                                        </option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="mb-5">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="without_sales">
                                <label class="form-check-label">
                        ...
                        <div class="mb-5">
                            ...
                            <input class="form-control" name="from" type="datetime-local" required>
                        </div>

                        <div class="mb-2">
                            <label class="form-label">Дата и время окончания действия промокода:</label>
                            <input class="form-control" name="to" id="date_to" type="datetime-local" required>
                        </div>

                        <div class="mb-5">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="promocode_newer_ends"  name="promocode_newer_ends">
                                <label class="form-check-label">
